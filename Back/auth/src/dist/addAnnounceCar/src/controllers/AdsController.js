...
// Настраиваем ImageKit (предполагаем, что переменные окружения настроены)
...
let AdsController = class AdsController {
    ...
    registerRoutes() {
        // Маршрут создания объявления с загрузкой изображений
        this.router.post("/", upload.array("photos"), // ожидаем, что в форме input имеет name="photos"
        [
        ...
        // Остальные маршруты (список, получение, обновление, удаление) остаются без изменений
        this.router.get("/", [
    ...
    createAd(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                // Если файлы загружены, отправляем их в ImageKit и получаем URL
                let imageUrls = [];
                // req.files может быть undefined или массивом файлов
                if (req.files && Array.isArray(req.files)) {
                    imageUrls = yield Promise.all(req.files.map((file) => __awaiter(this, void 0, void 0, function* () {
                        const result = yield imagekit.upload({
                            file: file.path, // путь к файлу
                            fileName: file.originalname,
                        });
                        return result.url;
                    })));
                }
                const data = Object.assign(Object.assign({}, req.body), { options: Array.isArray(req.body.options) ? req.body.options : [req.body.options], purchaseDate: new Date(req.body.purchaseDate), userId: userId, images: imageUrls });
